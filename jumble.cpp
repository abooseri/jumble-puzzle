...
{
	const string &hiddenString = hidden;

	// throw error if string is too big
	int hiddenSize = hidden.length();
	if (hiddenSize > 10)
		throw BadJumbleException("String entered is too large");
	if (hiddenSize < 3)
		throw BadJumbleException("String entered is too small");
